...
class DashboardService:
    ...
    def _calc_deadline_minutes(self, current_time: datetime.datetime) -> int:
        ...
        if self.order_service.product_group_deadlines:
            ...
            try:
                ...
                deadline_dt = current_time.replace(
    ...
    def get_home_overview(self, payload: dict, raw_data: dict) -> HomeDashboardResponse:
        ...
        inv_df = pd.DataFrame(
            raw_data.get("inventory_data", []),
            columns=(
                ["MASKED_STOR_CD", "STOCK_DT", "ITEM_CD", "ITEM_NM", "STOCK_QTY"]
                if not raw_data.get("inventory_data")
                else None
            ),
        )
        prod_df = pd.DataFrame(
            raw_data.get("production_data", []),
            columns=(
                ["MASKED_STOR_CD", "PROD_DT", "ITEM_CD", "PROD_QTY"]
                if not raw_data.get("production_data")
                else None
            ),
        )
        sales_df = pd.DataFrame(
            raw_data.get("sales_data", []),
            columns=(
                ["MASKED_STOR_CD", "SALE_DT", "ITEM_CD", "SALE_QTY", "TMZON_DIV"]
                if not raw_data.get("sales_data")
                else None
            ),
        )
        ...
        try:
            ...
            dash_res = type(
                ...
                {
                    "summary": type(
                        "obj", (object,), {"critical_count": 0, "avg_chance_loss_reduction": 0.0}
                    ),
                    "sku_list": [],
                },
            )
        ...
        ordering_status = "검토 필요" if order_deadline_min <= 60 else "안정"
        sales_health = "주의" if critical_count > 0 or order_deadline_min <= 30 else "안정"
        ...
        stats = [
            DashboardStat(
                ...
                tone="danger" if critical_count > 0 else "default",
            ),
            DashboardStat(
                key="ordering_deadline_minutes",
                label="주문 마감까지",
                value=f"{order_deadline_min}분",
                tone="primary",
            ),
            DashboardStat(
                ...
                label="오늘 순이익 추정",
                value=sales_health,
                tone="danger" if sales_health == "주의" else "success",
            ),
            DashboardStat(
                key="alert_count",
                label="알림 상태",
                value=f"긴급 {critical_count}건" if critical_count > 0 else "정상",
                tone="danger" if critical_count > 0 else "default",
            ),
        ]
            ...
            cards=[
                DashboardCard(
                    ...
                    highlights=(
                        ...
                        if top_item is not None
                            ...
                            CardHighlight(
                                title="위험 SKU 없음",
                                description="현재 품절 위험 상품이 감지되지 않았습니다.",
                                tone="info",
                            )
                        ]
                    ),
                    metrics=[
                        ...
                        CardMetric(
                            label="찬스 로스 절감",
                            value=f"{avg_reduction:.1f}%" if avg_reduction else "-",
                            tone="primary",
                        ),
                    ],
                ),
                DashboardCard(
                    ...
                    highlights=[
                        CardHighlight(
                            title="주문 마감 임박",
                            description=f"{order_deadline_min}분 남음 · AI 추천안 3개 준비됨",
                            tone="warning",
                    ...
                    metrics=[
                        CardMetric(label="주문 상태", value=ordering_status, tone="default"),
                        CardMetric(
                            label="AI 추천안",
                            value="3개 준비됨" if order_deadline_min <= 60 else "확인 필요",
                            tone="primary",
                        ),
                ...
                DashboardCard(
                    ...
                    highlights=[
                        CardHighlight(
                            title="오늘 순이익 추정",
                            description="손익 분석 화면에서 실 데이터 기반 분석을 확인하세요.",
                            tone="info",
                        ),
                        CardHighlight(
                            title="매출 인사이트 조회 가능",
                            description="AI 질의로 채널별·상품별 매출 분석을 제공합니다.",
                            tone="info",
                        ),
                    ],
                    metrics=[
        ...
        insights_data = DashboardInsightsResponse(
            insights=[
                InsightItem(
                    ...
                    description=production_insight,
                    evidence_sources=["생산 예측 엔진 (실 DB 기반)"],
                ),
                InsightItem(
                    id="sales-query",
                    description="손익 분석 화면에서 AI 질의로 상품별·채널별 매출 인사이트를 확인할 수 있습니다.",
                    evidence_sources=["매출 분석 Agent"],
                ),
            ],
            quick_links=[
