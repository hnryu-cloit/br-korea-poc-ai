...
class SeasonalityEngine:
    ...
    def _compute_dow_weights(self, sales_df: pd.DataFrame | None) -> dict[int, float]:
        """
    ...
    def get_weight(self, target_date: str, item_id: str | None = None) -> float:
        ...
        if not self._prepared.empty:
            ...
            if item_id and "item_id" in self._prepared.columns:
                item_mask = (
                    self._prepared["item_id"].astype(str) == str(item_id)
                ) | self._prepared["item_id"].isna()
                mask = mask & item_mask
            matched = self._prepared[mask]
            if not matched.empty:
                weight = float(matched["weight"].max())
                logger.info(
                    "SeasonalityEngine: campaign weight date=%s item=%s weight=%.3f",
                    target_date,
                    item_id,
                    weight,
                )
        ...
        if self._dow_weights:
            ...
            logger.info(
                "SeasonalityEngine: dow weight date=%s dow=%d weight=%.3f", target_date, dow, weight
            )
            return weight
