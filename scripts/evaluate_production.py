...
def evaluate_inventory_reversal(agent: ProductionManagementAgent, store_id: str, item_id: str, target_date: str):
    """[과제 1] 재고 역산 엔진(Inventory Reversal) 정합성 검증"""
    print(f"\n" + "="*60)
    print(f"📊 [과제 1] 재고 역산 엔진 정합성 검증 (매장: {store_id}, 상품: {item_id}, 날짜: {target_date})")
    print("="*60)
    ...
    try:
        ...
        print(f"{'시간':^10} | {'기존재고':^10} | {'입고(생산)':^10} | {'출고(판매)':^10} | {'추정재고(역산)':^10}")
        print("-" * 60)
        ...
        if not negative_stock.empty:
            ...
            print("   -> 기초 재고 데이터가 부족하거나 출고(판매) 데이터가 입고(생산)보다 먼저/많이 기록된 경우입니다.")
        else:
...
def evaluate_prediction_model(agent: ProductionManagementAgent, store_id: str, item_id: str, target_date: str):
    """[과제 2] 예측 모델(ML) 베이스라인 성능 평가"""
    print(f"\n" + "="*60)
    print(f"🎯 [과제 2] ML 예측 모델(Predictor) 성능 평가")
    print("="*60)
    ...
    try:
        ...
        print(f"{'기준시간':^15} | {'예측 판매량(AI)':^15} | {'실제 판매량(DB)':^15} | {'오차(Error)':^10}")
        print("-" * 60)
        ...
        for hour in test_hours:
            ...
            pred_qty = agent.predictor.predict_next_hour_sales(store_id, item_id, current_time, sales_df)
            ...
            actual_qty = pd.to_numeric(actual_sales_row['SALE_QTY'], errors='coerce').fillna(0).sum() if not actual_sales_row.empty else 0
            
            error = abs(pred_qty - actual_qty)
            total_error += error
            valid_count += 1
            
            print(f"{current_time.strftime('%H:%M')}->{next_hour_time.strftime('%H:%M')} | {pred_qty:^15.1f} | {actual_qty:^15.1f} | {error:^10.1f}")
        ...
        if valid_count > 0:
            ...
            if mae < 3.0:
                print("✅ 훌륭함: 평균적으로 실제 판매량과 3개 이내의 오차를 보이며 예측하고 있습니다.")
            else:
                print("⚠️ 주의: 예측 오차가 다소 높습니다. 모델 하이퍼파라미터 튜닝이 필요할 수 있습니다.")
...
if __name__ == "__main__":
    ...
    # 동적으로 가장 많이 팔린 상품 찾기
    top_item = sales_df[sales_df['SALE_DT'].astype(str) == TEST_TARGET_DATE].groupby('ITEM_CD')['SALE_QTY'].sum().idxmax()
    if pd.isna(top_item):
